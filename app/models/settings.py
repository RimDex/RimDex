...
class Settings(QObject):
    ...
    def __init__(self) -> None:
        ...
        self.external_steam_metadata_url: str = "https://github.com/RimDex/Steam-Workshop-Database/archive/refs/heads/main.zip"
        ...
        self.external_community_rules_url: str = "https://github.com/RimDex/Community-Rules-Database/archive/refs/heads/main.zip"
        ...
        # Database Expiry disabled by default previously this was 7 days "604800"
    ...
    def handle_corrupted_settings(self) -> None:
        """Report a corrupted settings file to the UI via the EventBus.

        The model layer no longer imports UI code; the controller subscribed to
        ``settings_corrupted`` shows the dialog, recovers the backup, or exits.
        """
        use_old_backup = False
        backup_path = AppInfo().settings_backups_folder / "settings.json.backup"
        old_backup_path = AppInfo().settings_backups_folder / "settings.json.backup.old"

        if backup_path.exists():
            text = self.tr(
                "Your settings file seems to be corrupted and cannot be loaded. "
                "RimDex found a backup at {}. "
                "Do you want to attempt to recover your settings from this backup?"
            ).format(backup_path)
        elif old_backup_path.exists():
            text = self.tr(
                "Your settings file seems to be corrupted and cannot be loaded. "
                "RimDex found an old backup at {}. "
                "Do you want to attempt to recover your settings from this backup?"
            ).format(old_backup_path)
            use_old_backup = True
        else:
            text = self.tr(
                "Your settings file seems to be corrupted and cannot be loaded. "
                "No backup file was found, RimDex will reset your settings to defaults."
            )

        info_text = self.tr(
            "If you proceed, a backup of the corrupted file will be saved to {}."
        ).format(AppInfo().settings_backups_folder / "settings.json.corrupted")

        failure_text = self.tr(
            "RimDex failed to recover your settings from the backup. "
            'You may be able to manually recover your settings by restoring "settings.json.backup" or "settings.json.backup.old" from {} to {}.'
        ).format(AppInfo().settings_backups_folder, self._settings_file)

        EventBus().settings_corrupted.emit(
            {
                "title": self.tr("Settings Load Error"),
                "text": text,
                "information": info_text,
                "positive_text": self.tr("Proceed"),
                "negative_text": self.tr("Exit RimDex"),
                "use_old_backup": use_old_backup,
                "failure_title": self.tr("Settings Recovery Failed"),
