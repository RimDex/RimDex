...
class Settings(QObject):
    ...
    def handle_corrupted_settings(self) -> None:
        ...
        if (AppInfo().settings_backups_folder / "settings.json.backup").exists():
            msg += (
                f"RimDex found a backup at {AppInfo().settings_backups_folder / 'settings.json.backup'}. "
            ...
            msg += (
                f"RimDex found an old backup at {AppInfo().settings_backups_folder / 'settings.json.backup.old'}. "
                "Do you want to attempt to recover your settings from this backup?"
            )
            use_old_backup = True
        else:
            msg += "No backup file was found, RimDex will reset your settings to defaults."
