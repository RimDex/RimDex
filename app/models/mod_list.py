...
class MetadataProvider(Protocol):
    """Structural interface for the metadata access leaf layers require.

    Implemented by ``MetadataController``; declared here (a leaf-safe module)
    so other leaf layers (``services/``, ``utils/*``) can type against it
    without importing ``app/controllers``. Members are read-only (mirroring
    ``MetadataController``'s properties).
    """
    ...
    @property
    def mods_metadata(self) -> dict[str, Any]: ...

    @property
    def steam_db(self) -> Any: ...

    @property
    def game_version(self) -> str: ...

    def get_mod(self, path: Any) -> Any: ...
