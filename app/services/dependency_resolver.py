...
class MetadataProvider(Protocol):
    """Minimal view onto ``MetadataController`` needed by the resolver.

    Declared as a Protocol so ``app.services`` (a leaf layer) does not have to
    import ``app.controllers`` (enforced by ``check_layer_violations.py``). Any
    object exposing ``mods_metadata``, ``settings``, ``steam_db`` and
    ``game_version`` — e.g. a ``MetadataController`` instance — satisfies it.

    Members are read-only properties: ``MetadataController`` exposes them as
    read-only attributes, and mypy requires a Protocol member to be at least as
    settable as the candidate (so read-only avoids false mismatches).
    """

    @property
    def mods_metadata(self) -> Any: ...

    @property
    def settings(self) -> Settings: ...

    @property
    def steam_db(self) -> Any | None: ...
...
def parse_workshop_id_from_url(url: str) -> str | None:
    """Extract a numeric Steam Workshop ID from a dependency URL.

    The value originates from an installed mod's ``About.xml``
    ``<steamWorkshopUrl>`` text, which is untrusted. Only purely-numeric IDs
    are accepted: any whitespace or control characters are stripped first so a
    crafted URL cannot inject extra lines into a SteamCMD download script (see
    ``steamcmd/wrapper._build_download_script``).
    """
    if "?id=" in url:
        ...
        return None
    # Drop query/fragment/path separators, then keep digits only.
    workshop_id = workshop_id.split("?", 1)[0]
    workshop_id = workshop_id.split("/", 1)[0]
    workshop_id = workshop_id.split("&", 1)[0]
    workshop_id = re.sub(r"\D", "", workshop_id)
