"""Parse RimWorld and RimDex mod list files."""
...
from app.core.schema import validate_rimworld_mods_list
from app.io.xml import xml_path_to_json
from app.models.metadata.metadata_factory import value_extractor

SourceFormat = Literal["mods_config_xml", "rimdex_json", "rml", "rws", "savegame"]
...
def _parse_xml_mod_list(path: Path, data: dict[str, Any]) -> ParsedModList:
    ...
    # Always delegate to validate_rimworld_mods_list: it handles every supported
    # RimWorld XML layout (ModsConfigData.activeMods, savedModList.meta.modIds,
    # savegame.meta.modIds). Gating on activeMods here would wrongly return an
    # empty list for .rml/.rws/savegame files and risk overwriting the live
    # ModsConfig.xml with nothing on import.
...
def parse_mod_list_file(path: str | Path) -> ParsedModList:
    ...
    raise ModListFormatError(
        "Unrecognized mod list format. Expected RimWorld ModsConfig XML or RimDex JSON."
    )
