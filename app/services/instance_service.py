...
from app.core.generic import handle_remove_read_only
from app.models.instance import Instance, InvalidArchivePathError
...
class InstanceControllerProtocol(Protocol):
    """Interface the instance service needs from an InstanceController."""
    ...
    def extract_from_archive(
        self, archive_path: str, delete_old: bool = True
    ) -> None: ...
    ...
    @classmethod
    def from_archive(cls, archive_path: str) -> "InstanceControllerProtocol": ...


class AuxMetadataControllerProtocol(Protocol):
    """Interface the instance service needs from an AuxMetadataController."""
...
class InstanceService:
    ...
    def __init__(
        ...
        instance_controller_cls: type[InstanceControllerProtocol],
        aux_metadata_controller: AuxMetadataControllerProtocol,
    ) -> None:
        self.settings = settings
        self.steamcmd_wrapper = steamcmd_wrapper
        self._instance_controller_cls = instance_controller_cls
        self._aux_metadata_controller = aux_metadata_controller
    ...
    def backup_existing_instance(self, instance_name: str) -> None:
        ...
        if instance_name == DEFAULT_INSTANCE_NAME:
            while True:
                ...
                if new_instance_name.lower() != DEFAULT_INSTANCE_NAME.lower():
                    break
                show_warning(
                    title=QCoreApplication.translate("InstanceService", "Invalid name"),
                    text=QCoreApplication.translate(
                        "InstanceService",
                        '"{name}" is not allowed. Please choose a different name.',
                    ).format(name=DEFAULT_INSTANCE_NAME),
                )
        ...
        instance_controller = self._instance_controller_cls(instance)
    ...
    def restore_instance_from_archive(self) -> None:
        ...
        try:
            instance_controller = self._instance_controller_cls.from_archive(input_path)
    ...
    def clone_existing_instance(self, existing_instance_name: str) -> None:
        ...
        if (
            ...
            new_instance_path = str(
                Instance.get_instance_folder_path(
                ...
                if existing_instance_workshop_folder:
                    _answer = show_dialogue_conditional(
                        ...
                        information=QCoreApplication.translate(
                            ...
                            "RimDex will copy all Workshop mods to the new"
                    ...
                    if _answer == QMessageBox.StandardButton.Cancel:
                        show_warning(
                            title=QCoreApplication.translate(
                                "InstanceService", "Workshop folder not configured"
                            ),
                            text=QCoreApplication.translate(
                                "InstanceService",
                                "Workshop folder was not configured for the"
                                " cloned instance. You can set it later in settings.",
                            ),
                        )
                    elif _answer == QCoreApplication.translate(
                        "InstanceService", "Convert to SteamCMD"
                    ):
                        # Note: when `button_text_override` is used, the return is the
                        # button text string, not a StandardButton enum.
                        if os.path.exists(
    ...
    def create_new_instance(
        ...
        if (
            ...
            instance_path = Instance.get_instance_folder_path(
    ...
    def delete_current_instance(self) -> None:
        ...
        if self.settings.current_instance == DEFAULT_INSTANCE_NAME:
            ...
            if answer.exec_is_positive():
                try:
                    self._aux_metadata_controller.engine.dispose()
                except Exception as e:
                    logger.error(f"Error disposing aux database: {e}")
                try:
                    rmtree(
                        str(
                            Path(
                    ...
                    logger.error(f"Error deleting instance files: {e}")
                    return
                self.settings.instances.pop(self.settings.current_instance)
                self.settings.current_instance = DEFAULT_INSTANCE_NAME
                self.settings.save()
