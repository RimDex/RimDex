...
from app.core.fs_utils import scanpath
...
def _get_path_to_color_map(
    ...
    return _get_path_to_field_map(paths, settings, "color_hex")
...
def _get_path_to_updated_map(
    ...
    return _get_path_to_field_map(paths, settings, "acf_time_updated")


def _get_path_to_field_map(
    paths: list[str],
    settings: Settings,
    attr: str,
) -> dict[str, Any]:
    """Bulk-fetch a single ``AuxMetadataEntry`` attribute for a batch of paths.

    Uses ``get_all_by_paths`` (single IN query) instead of N individual
    ``get()`` calls.

    :param paths: List of mod paths
    :param settings: Settings instance
    :param attr: Name of the ``AuxMetadataEntry`` column to read
    :return: Dict mapping path -> attribute value (truthy values only)
    """
    result: dict[str, Any] = {}
    try:
        ...
        with aux_controller.Session() as aux_session:
            entries_by_path = aux_controller.get_all_by_paths(aux_session, paths)
            for path in paths:
                entry = entries_by_path.get(path)
                val = getattr(entry, attr, None) if entry else None
                if val:
                    result[path] = val
    except Exception as e:
        logger.warning(f"Failed to fetch {attr} from auxiliary metadata: {e}")
    return result
