...
def _new_session(allowed_methods: frozenset[str] | None = None) -> requests.Session:
...
def _request(
    ...
    url: str,
    *,
    _allowed_methods: frozenset[str] | None = None,
    **kwargs: Any,
) -> requests.Response:
    """Send a request through a Session with a default timeout."""
    kwargs.setdefault("timeout", DEFAULT_TIMEOUT)
    with _new_session(allowed_methods=_allowed_methods) as session:
...
def post(url: str, *, retry: bool = False, **kwargs: Any) -> requests.Response:
    """Perform a POST request with a default timeout.

    Args:
        url: The URL to send the request to.
        retry: If True, transient failures (connection errors, 429, 5xx)
            are retried automatically using exponential backoff.
        **kwargs: Additional arguments passed to requests.Session.request().
    """
    if retry:
        return _request("POST", url, _allowed_methods=frozenset(["POST"]), **kwargs)
