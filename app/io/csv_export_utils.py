...
from app.ui.dialogue import show_dialogue_file, show_information, show_warning
...
def export_to_csv(panel: BaseModsPanel) -> None:
    ...
    try:
        ...
        _handle_csv_export_error(
            str(e),
            QCoreApplication.translate("CsvExportUtils", "Invalid File Path"),
        )
    except PermissionError:
        _handle_csv_export_error(
            EXPORT_PERMISSION_DENIED,
            QCoreApplication.translate("CsvExportUtils", "Export Permission Denied"),
        )
    except OSError as e:
        _handle_csv_export_error(
            EXPORT_FILESYSTEM_ERROR.format(e=str(e)),
            QCoreApplication.translate("CsvExportUtils", "Export File System Error"),
        )
    except Exception as e:
        _handle_csv_export_error(
            EXPORT_UNKNOWN_ERROR,
            QCoreApplication.translate("CsvExportUtils", "Export Unknown Error"),
            str(e),
        )
...
def _finalize_csv_export(panel: BaseModsPanel, file_path: str) -> None:
    ...
    show_information(
        title=QCoreApplication.translate("CsvExportUtils", "Export Success"),
        text=QCoreApplication.translate(
            "CsvExportUtils", "Successfully exported {count} items to {file_path}"
        ).format(count=row_count, file_path=file_path),
    )
...
def _handle_csv_export_error(
    ...
    Args:
        ...
        title: The (already-translated) dialog title.
...
def _get_table_model(panel: BaseModsPanel) -> Any:
    ...
    return (
        ...
        else panel.editor_table_view.model()
    )
