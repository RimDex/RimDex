...
class RunnerPanel(QWidget):
    ...
    def __init__(
        ...
        # Continuous progress across phases
    ...
    def execute(
        ...
        # SteamCMD console_log.txt tail (live logs on Windows)
        # SteamCMD writes its console log to <install_dir>/logs/console_log.txt
        if "steamcmd" in command:
            self._steamcmd_console_log_path = str(
                Path(command).resolve().parent / "logs" / "console_log.txt"
            )
    ...
    def _handle_query_output(self, line: str) -> bool:
        ...
        if match:
            _operation, _pagination, start_str, end_str = match.groups()
            start, end = int(start_str), int(end_str)
            if self.progress_bar.isHidden():
                self.progress_bar.show()
            # Detect new phase (value resets to 0 with a smaller range)
            # and accumulate offset so the progress bar grows continuously
            # across phases (GetDetails round 1 → round 2 → Steamworks).
            if start == 0 and end <= self.progress_bar.maximum():
                self._progress_offset += self.progress_bar.maximum()
            self.progress_bar.setRange(0, end + self._progress_offset)
            self.progress_bar.setValue(start + self._progress_offset)
            return True
