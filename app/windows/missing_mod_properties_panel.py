...
from app.core.constants import DEFAULT_MISSING_PACKAGEID
...
class MissingModPropertiesPanel(BaseModsPanel):
    ...
    def _add_to_ignore_list(self) -> None:
        ...
        if not selected_indices:
            self._show_message(
                ...
                self.tr("Please select mods to add to the ignore list."),
                "information",
            )
            return
    ...
    def _process_ignore_list_addition(self, selected_indices: Iterable[int]) -> bool:
        ...
        if not IgnoreManager.add_ignored_mods(packageids_to_add):
            ...
            self._show_message(
                self.tr("Error"),
                self.tr("Failed to add mods to ignore list."),
                "critical",
            )
            return False
        ...
        self._show_message(
            self.tr("Success"),
            self.tr("Mods added to ignore list. Panel will refresh."),
            "information",
        )
        return True
    ...
    def _show_no_valid_packageids_warning(self, skipped_mods: list[str]) -> None:
        ...
        if skipped_mods:
            ...
            self._show_message(self.tr("Cannot Add"), message, "warning")
        else:
            self._show_message(
                self.tr("Error"),
                self.tr("Could not extract package IDs from selected mods."),
                "warning",
            )
