"""Apply / self-update layer for RimDex (Qt + UI wiring).

This module owns the *active* update flow: download, archive extraction, backup,
and launching the platform-specific update script / installer. It intentionally
imports PySide6 and :mod:`app.views` (progress UI), while the pure version /
asset-resolution logic lives in :mod:`app.core.update_check`.

:mod:`app.core.update_utils` re-exports the public symbols from both modules for
...
# Re-export the pure-layer names so ``from app.core.update_apply import UpdateManager``
# and ``from app.core.update_utils import UpdateError`` both keep working.
UpdateError = check.UpdateError
UpdateNetworkError = check.UpdateNetworkError
UpdateDownloadError = check.UpdateDownloadError
UpdateExtractionError = check.UpdateExtractionError
UpdateScriptLaunchError = check.UpdateScriptLaunchError
ReleaseInfo = check.ReleaseInfo
DownloadInfo = check.DownloadInfo
PlatformPatterns = check.PlatformPatterns
ScriptConfig = check.ScriptConfig
PLATFORM_PATTERNS = check.PLATFORM_PATTERNS
...
class TarExtractThread(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.finished.emit(
                True,
                f"{self.tar_path} â†’ {self.target_path}\n"
...
class UpdateManager(QObject):
    ...
    # Class-level cached platform patterns for performance (alias of the
    # canonical table in app.core.update_check).
    ...
    # cache state on the manager instance)
    # ------------------------------------------------------------------
    ...
    def _is_in_protected_path(self) -> bool:
        ...
        return check._is_in_protected_path()
    ...
    def _validate_prerequisites(self) -> bool:
        ...
        if os.getenv("RIMDEX_DISABLE_UPDATER"):
            logger.debug(
                "RIMDEX_DISABLE_UPDATER is set, skipping update check silently."
            )
            return False
    ...
    def _fetch_and_compare_versions(self) -> dict[str, Any] | None:
        ...
        # Compare versions
        current_version_parsed = check.parse_version(current_version)
        ...
        try:
            if current_version == "Unknown version":
                ...
                answer = dialogue.show_dialogue_conditional(
                    title=self.tr("Unknown Version Detected"),
                    text=self.tr(
                        "Could not find version Information of RimDex, This may indicate that the application is not an offical release and may be a custom build."
                    ),
                    information=self.tr(
                        "Are you sure you want to still update anyway?"
                    ),
                )
    ...
    def _prompt_user_for_update(
        ...
        answer = dialogue.show_dialogue_conditional(
            ...
            information=self.tr(
                "You are running RimDex {current_version}<br>Do you want to update now?"
    ...
    def _get_latest_release_info(
        ...
        Delegates the pure fetch/parse/asset-resolution to
        :func:`app.core.update_check.get_latest_release_info` and surfaces the
        failure cases (connection errors, no valid release) via dialogue so the
        caller behaviour is unchanged.

        Returns:
            Dictionary containing version, tag_name, download_url, and is_msi flag,
            or None if failed
        """
        try:
            return check.get_latest_release_info(needs_elevation)
        except requests.RequestException as e:
            logger.warning(f"Failed to fetch release information: {e}")
            dialogue.show_warning(
                title=self.tr("RimDex Update Error"),
            ...
            # Pure layer already logged / built the message; surface a generic one.
    ...
    # ------------------------------------------------------------------
    # Asset-selection methods â€” thin wrappers over the pure functions in
    # app.core.update_check (kept as methods so the existing test contract that
    # binds UpdateManager.<method>.__get__ keeps working).
    # ------------------------------------------------------------------
    ...
    def _asset_matches(
        ...
        return check.asset_matches(
            asset, patterns, extension, require_arch, arch_patterns
        )
    ...
    def _find_best_asset_match(
        ...
        return check.find_best_asset_match(
    ...
    def _find_appimage_asset(
        ...
        return check.find_appimage_asset(assets, arch_patterns)

    def _get_platform_download_url(
        self, assets: list[dict[str, Any]], needs_elevation: bool = False
    ) -> DownloadInfo | None:
        return check.resolve_platform_download_url(
            assets,
            self._system,
            self._arch,
            self._cached_patterns,
            AppInfo().is_appimage,
            self._is_in_protected_path(),
        )
    ...
    def _perform_update(
        ...
        try:
            ...
            # AppImage updates use .bak rename as the backup â€” skip ZIP backup
    ...
    def _get_file_size(self, url: str) -> int:
        ...
        """
        try:
            head_response = http.head(
                url, timeout=check.API_TIMEOUT, allow_redirects=True
            )
    ...
    def _download_with_progress(
        ...
        for chunk in response.iter_content(chunk_size=check.DOWNLOAD_CHUNK_SIZE):
            if self._download_cancelled:
                raise UpdateDownloadError("Download cancelled by user")
            if chunk:
    ...
    def _validate_download(self, content: bytes) -> None:
        ...
        if len(content) < check.MIN_UPDATE_SIZE:
            raise UpdateDownloadError(
    ...
    def _download_update(self, url: str) -> None:
        ...
        try:
            ...
            response = http.get(url, timeout=check.DOWNLOAD_TIMEOUT, stream=True)
    ...
    def _create_temp_dir(self) -> Path:
        ...
        temp_base = (
            Path(gettempdir())
            / f"RimDex_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}"  # noqa: DTZ005
        )
    ...
    def _extract_zip(self, content: bytes, temp_base: Path) -> int:
        ...
        try:
            ...
            extract_thread.wait(check.EXTRACTION_THREAD_TIMEOUT_MS)
            # jscpd:ignore-start
    ...
    def _prepare_appimage_update(self) -> Path:
        ...
        if not os.access(target_dir, os.W_OK):
            raise UpdateExtractionError(
                ...
                "Move RimDex to a user-writable location or update manually."
            )
    ...
    def _should_unwrap_directory(self, top_dir: Path) -> bool:
        ...
        if self._system == "Darwin":
            ...
            if top_dir_name in ["rimdex", "rimdex.app"]:
                return False
        ...
        if check.VERSION_PATTERN.match(top_dir_name):
            return True
        ...
        try:
            ...
            if AppInfo().is_appimage:
                fd, temp_script = tempfile.mkstemp(
                    suffix=".sh", prefix="rimdex_update_"
                )
                os.close(fd)
        ...
        try:
            if self._system == "Linux" and needs_elevation:
                raise Exception(  # noqa: TRY002
                    "sudo requires a terminal emulator on Linux"
                )
    ...
    def _prepare_update_log(self, system: str) -> Path:
        ...
        log_path = log_dir / check.UPDATER_LOG_FILENAME
    ...
    def _create_backup(self) -> None:
        ...
        try:
            ...
            answer = dialogue.show_dialogue_conditional(
                ...
                text=self.tr("Failed to create a backup before updating."),
                information=f"{e}",
    ...
    def _cleanup_old_backups(self) -> None:
        ...
        try:
            ...
            backup_files = list(app_backup_folder.glob("RimDex_Backup_*.zip"))
    ...
    def show_update_error(self) -> None:
        dialogue.show_warning(
            ...
            text=self.tr(
                "Please check your internet connection and try again, You can also check 'https://github.com/RimDex/RimDex/releases' directly."
            ),
        )
