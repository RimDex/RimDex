...
# ---------------------------------------------------------------------------
# Shared async translation helper
# ---------------------------------------------------------------------------
...
async def translate_one_async(
    ...
    """Translate a single unfinished item via the given service.

    Returns the translated text, or ``""`` on skip / failure.
    Shared by the CLI tool and the QThread worker to avoid code duplication.
    """
    ...
    try:
        ...
        result = await asyncio.wait_for(
            service.translate(item.source, target_lang, source_lang),
            timeout=timeout,
        )
        return result or ""
    except Exception as exc:
        logger.warning("Translation failed for [%s]: %s", item.source[:40], exc)
        return ""


# ---------------------------------------------------------------------------
# Shared batch translation
# ---------------------------------------------------------------------------


async def translate_language_batch(
    lang_code: str,
    service: Any,
    *,
    ...
    Single implementation shared by the CLI tool and the GUI worker.
    Handles backup, concurrent translation, XML element updates, and
    save-or-restore on failure.

    Args:
        lang_code: Target language code (e.g. ``"de_DE"``).
        service: A :class:`TranslationService` instance.
        on_progress: Optional callback for status messages.
        on_item_done: Optional callback ``(index, source, translation)``
            fired after each item is translated.
    ...
    ts_file = LOCALES_DIR / f"{lang_code}.ts"
    ...
    if not ts_file.exists():
        _error(f"File not found: {ts_file}")
        return 0, 0

    shutil.copy2(ts_file, backup_file)
    _progress(f"Backup created: {backup_file}")
    ...
    try:
        ...
        if not unfinished:
            _progress(f"No unfinished translations for {lang_code}")
            if backup_file.exists():
                backup_file.unlink()
            return 0, 0

        _progress(f"Found {len(unfinished)} unfinished translations")
        ...
        async def _translate_one(i: int, item: UnfinishedItem) -> tuple[int, str]:
            async with semaphore:
                text = await translate_one_async(
                    item,
                    lang_code,
                    "en_US",
                    service,
                    config.timeout_config.default_timeout,
                )
                return i, text
        ...
        for i, translated_text in results:
            item = unfinished[i]
            # Skip trivial items (empty source or single char / digit / punctuation)
            if should_skip_translation(item.source):
                continue
            ...
            if translated_text:
                ...
                if is_plural:
                    ...
                    if forms:
                        forms[0].text = translated_text
                else:
                    item.element.text = translated_text
                item.element.attrib.pop("type", None)
                if on_item_done:
                    on_item_done(i, item.source, translated_text)

            # Empty result for a non-trivial source counts as a failure
            if not translated_text:
                failed += 1

        if failed == 0 or config.use_cache:
            save_ts_file(tree, ts_file)
            get_translation_cache().save()
            _progress(f"Saved {translated} translations to {ts_file}")
            if backup_file.exists():
                backup_file.unlink()
        else:
            _error(f"Translation failed for {lang_code} — restoring backup")
            shutil.copy2(backup_file, ts_file)

        return translated, failed

    except Exception as e:
        _error(f"Translation batch failed: {e}")
        if backup_file.exists():
            shutil.copy2(backup_file, ts_file)
            _progress("Restored from backup")
        return 0, 0
