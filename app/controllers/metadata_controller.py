...
from app.core.schema import generate_rimworld_mods_list, validate_rimworld_mods_list
from app.io.acf_utils import load_acf_from_path
...
from app.mods.ignore_manager import IgnoreManager
...
class MetadataController(QObject):
    ...
    def get_metadata_with_path(
        ...
        Triggers a lazy load of the ``tags`` relationship while the session
        is still open so that ``entry.tags`` (and ``entry.__dict__["tags"]``)
        are usable after the session is closed.
        ...
        with self.metadata_db_controller.Session() as session:
            entry = self.metadata_db_controller.get_or_create(session, path)
            # Eager-load tags while session is still open
    ...
    def get_missing_packageid_paths(self) -> list[str]:
        ...
        """
        return self.get_missing_properties_paths()[0]

    def get_missing_publishfieldid_paths(self) -> list[str]:
        """Identify mods lacking a Publish Field ID (Steam Workshop ID)."""
        return self.get_missing_properties_paths()[1]

    def get_missing_properties_paths(self) -> tuple[list[str], list[str]]:
        """Single-pass equivalent of both getters above.

        :returns: ``(missing_packageid_paths, missing_publishfieldid_paths)``
        """
        ...
        for path, mod in self.mods_metadata.items():
            ...
            if isinstance(mod, AboutXmlMod):
                ...
                if pid in ignored_mods:
                    continue
            missing_publishfieldid.append(path)
        logger.info(
            "Missing properties: {} missing PackageId, {} missing PublishedFileId",
            len(missing_packageid),
            len(missing_publishfieldid),
        )
        return missing_packageid, missing_publishfieldid
