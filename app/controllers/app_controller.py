...
class AppController(QObject):
    def __init__(self) -> None:
        ...
        self.app.setDesktopFileName("io.github.rimdex.RimDex")
    ...
    def initialize_settings(self) -> None:
        ...
        EventBus().settings_corrupted.connect(self._on_settings_corrupted)
        self.settings.load()
    ...
    def _on_settings_corrupted(self, payload: dict[str, Any]) -> None:
        """Handle a corrupted-settings report from the model (via EventBus).

        Shows the recovery dialog and either recovers the backup or exits. The
        model layer stays free of UI/process-exit concerns.
        """
        ...
        if dlg.exec_is_positive():
            try:
                self.settings.recover_backup(use_old_backup=payload["use_old_backup"])
            except Exception as e:
                logger.error(f"Failed to recover settings from backup: {e}")
                InformationBox(
                    title=payload["failure_title"],
                    text=payload["failure_text"],
                ).exec()
        else:
            sys.exit(0)
    ...
    def initialize_instance_service(self) -> None:
        ...
        self.instance_service = InstanceService(
            ...
            instance_controller_cls=InstanceController,
