...
class DatabaseUploadHandler:
    ...
    def __init__(
        self, settings: Settings, tr: Callable[..., str], git_ops_handler: GitOpsHandler
    ) -> None:
    ...
    def _clone_callback(self, base_path: str, repo_url: str) -> None:
        """Clone the database repository when it is missing locally."""
        ...
        try:
            with git_utils.git_repository(repo_path, config) as repo:
                ...
                if stash_created:
                    ...
                    unstash_result = git_utils.git_stash(repo, pop=True, config=config)
        ...
        try:
            ...
            if answer == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    logger.exception(f"Failed to open browser with error: {e}")
