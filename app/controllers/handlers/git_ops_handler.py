...
class GitOpsHandler:
    ...
    def handle_batch_push_results(self, results: GitBatchPushResults) -> None:
        ...
        self._show_batch_results(
            ...
            failure_information=self._tr("{count} repositories could not be pushed.").format(
                count=len(failed)
            ),
            failure_details=failure_details,
    ...
    def do_git_clone(self, base_path: str, repo_url: str) -> None:
        ...
        binary_diag = BinaryChoiceDialog(
            ...
            information=self._tr("Repository: {repo_url}<br>Destination: {dest}").format(
        ...
        if full_repo_path.exists():
            answer = show_dialogue_conditional(
                ...
                button_text_override=[self._tr("Clone new"), self._tr("Update existing")],
            )
