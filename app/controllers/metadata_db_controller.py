...
class AuxMetadataController(MetadataDbController):
    ...
    @staticmethod
    def get_all_by_paths(
        session: Session, paths: list[str]
    ) -> dict[str, "AuxMetadataEntry"]:
        """Bulk-fetch aux metadata entries for a list of paths (single query).

        Eagerly loads the ``tags`` relationship to avoid per-item lazy loads
        on detached instances.

        :param session: The database session.
        :param paths: List of mod paths to fetch.
        :return: Dict mapping path -> AuxMetadataEntry.
        """
        entries = (
            session.query(AuxMetadataEntry)
        ...
        logger.debug(
            "Bulk-fetched {}/{} aux entries with tags",
            len(result),
            len(paths),
        )
        return result
