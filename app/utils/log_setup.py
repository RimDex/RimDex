"""Centralized loguru logging configuration for RimDex."""
...
def _rotate_session_logs(log_dir: Path, base_name: str) -> None:
    ...
    Cascades existing logs upward (RimDex.log -> RimDex.1.log -> RimDex.2.log)
    and deletes logs beyond the history limit.

    :param log_dir: Directory containing log files
    :param base_name: Base log filename (e.g. "RimDex.log")
    """
...
def _formatter(record: loguru_module.Record) -> str:
    ...
    if exc is not None:
        ...
        exception_text = "".join(format_exception(exc_type, exc_value, exc_tb))
        exception_text = _obfuscate_message(exception_text)
...
def setup_logging(
    ...
    :param app_name: Application name, used to derive the log file name
    """
    logger.remove()

    log_dir.mkdir(parents=True, exist_ok=True)

    _rotate_session_logs(log_dir, f"{app_name}.log")
