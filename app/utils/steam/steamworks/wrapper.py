...
from app.core.game_launch import launch_game_process
...
def _create_steamworks(libs_path: str | None = None) -> STEAMWORKS:
    """Create a STEAMWORKS instance, temporarily chdir to *libs_path* if given.

    Some Steamworks DLLs (e.g. the Nuitka-compiled variant) ignore the
    ``_libs`` constructor kwarg and load from the current working directory.
    This helper chdirs to *libs_path* (copying ``steam_appid.txt`` if needed),
    constructs ``STEAMWORKS()``, restores CWD, and cleans up the temporary copy.

    Args:
        libs_path: Optional path to the directory containing the Steamworks native libraries.

    Returns:
        A new ``STEAMWORKS`` instance.
    """
    if libs_path:
        old_cwd = os.getcwd()
        steam_appid_src = os.path.join(old_cwd, "steam_appid.txt")
        steam_appid_dst = os.path.join(libs_path, "steam_appid.txt")
        need_copy = not os.path.isfile(steam_appid_dst) and os.path.isfile(
            steam_appid_src
        )
        try:
            if need_copy:
                copy2(steam_appid_src, steam_appid_dst)
            os.chdir(libs_path)
            return STEAMWORKS()
        finally:
            os.chdir(old_cwd)
            if need_copy:
                try:
                    os.remove(steam_appid_dst)
                except PermissionError:
                    logger.debug(
                        "Could not remove steam_appid.txt (in use by another process)"
                    )
    return STEAMWORKS()
...
class SteamworksInterface:
    ...
    Attributes:
        ...
        end_callbacks (Event): Signal to end the callback thread
    ...
    def __init__(
        ...
        if self.callbacks:
            ...
            self.end_callbacks = (
                threading.Event()
            )  # Signal used to end the _callbacks Thread
            if (
        ...
        self._app_deps_raw_callback: Any = None
        self.steam_not_running = False  # Skip action if True. Log occurrences.
        self.steamworks = _create_steamworks(_libs)
        try:
        ...
        if not self.steam_not_running and self.callbacks:  # Skip if True
            # Start the thread
    ...
    def _cb_app_dependencies_result_callback(self, *args: Any, **kwargs: Any) -> None:
        ...
        try:
            ...
            if pfid is not None and len(app_dependencies_list) > 0:
    ...
    def _wait_for_callbacks(self, timeout: int) -> bool:
        ...
        while not self.end_callbacks.is_set() and self.steamworks_thread.is_alive():
...
class SteamworksAppDependenciesQuery:
    ...
    def run(self) -> None | dict[int, Any]:
        ...
        if si is not None and not si.steam_not_running and si.end_callbacks.is_set():
        ...
        si._app_deps_raw_callback = CFUNCTYPE(None, GetAppDependenciesResult)(
            si._cb_app_dependencies_result_callback
        )
        si.steamworks.Workshop_SetGetAppDependenciesResultCallback(  # pyright: ignore
            si._app_deps_raw_callback
        )
        ...
        for idx, pfid in enumerate(self.pfid_or_pfids):
            si.steamworks.Workshop_GetAppDependencies(pfid)  # pyright: ignore
        ...
        if si.steamworks_thread.is_alive() and si.end_callbacks.is_set():
