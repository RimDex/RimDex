...
def check_if_pfids_blacklisted(
    ...
    for publishedfileid in publishedfileids:
        entry = steamdb.get(publishedfileid)
        if entry is None:
            entry = steamdb.get(str(publishedfileid))
        if entry is None:
            continue
    ...
    if blacklisted_mods:
        ...
        if answer == QMessageBox.StandardButton.Cancel or skip_text in answer_str:
...
def import_steamcmd_acf_data(
    ...
    if acf_to_import_path and os.path.exists(acf_to_import_path):
        ...
        return
    # Output
    workshop_installed = steamcmd_appworkshop_acf.get("AppWorkshop", {}).get(
        "WorkshopItemsInstalled", {}
    )
    workshop_details = steamcmd_appworkshop_acf.get("AppWorkshop", {}).get(
        "WorkshopItemDetails", {}
    )
    import_installed = acf_to_import.get("AppWorkshop", {}).get(
        "WorkshopItemsInstalled", {}
    )
    import_details = acf_to_import.get("AppWorkshop", {}).get("WorkshopItemDetails", {})
    items_installed_before = len(workshop_installed)
    logger.debug(f"WorkshopItemsInstalled beforehand: {items_installed_before}")
    item_details_before = len(workshop_details)
    logger.debug(f"WorkshopItemDetails beforehand: {item_details_before}")
    recursively_update_dict(workshop_installed, import_installed)
    recursively_update_dict(workshop_details, import_details)
    items_installed_after = len(workshop_installed)
    logger.debug(f"WorkshopItemsInstalled after: {items_installed_after}")
    item_details_after = len(workshop_details)
