...
from app.core.constants import RIMWORLD_DLC_METADATA
from app.core.generic import chunks
...
from app.ui.dialogue import show_warning
...
class CollectionImport:
    ...
    def input_dialog(self) -> None:
        ...
        self.link_input = QInputDialog.getText(
            ...
            QCoreApplication.translate(
                "CollectionImport", "Add Workshop collection link"
            ),
            QCoreApplication.translate(
                "CollectionImport", "Add Workshop collection link"
            ),
    ...
    def import_collection_link(self) -> None:
        ...
        if not self.is_valid_collection_link(collection_link):
            ...
            show_warning(
                title=QCoreApplication.translate("CollectionImport", "Invalid Link"),
                ...
                if failed_mods:
                    show_warning(
                        title=QCoreApplication.translate(
                            "CollectionImport", "Incomplete import"
                        ),
                        text=QCoreApplication.translate(
                            "CollectionImport",
...
class DynamicQuery(QObject):
    ...
    def __init__(
        ...
        self._steam_pool: MultiprocessingPool | None = (
            None  # Pool instance, set in _setup_steamworks_pool
        )
        self._steam_pool_ready = Event()
        self._steam_pool_error: str | None = None

    def _steamworks_status(self) -> str:
        if not self.get_appid_deps:
            return ""
        if not self._steam_pool_ready.is_set():
            return " | Steamworks: setting up..."
        if self._steam_pool_error:
            return " | Steamworks: n/a"
        if self._steam_pool is not None:
            return " | Steamworks: ready"
        return ""
    ...
    def _process_mod_details(
        ...
        missing_children = [
            ...
            if not (
                result["database"].get(mc) and result["database"][mc].get("unpublished")
            )
        ]
    ...
    def _setup_steamworks_pool(
        self, num_processes: int, libs_path: str, project_root: str
    ) -> None:
        from app.utils.steam.availability import check_steam_available
        from app.utils.steam.steamworks.wrapper import _pool_init_worker
        ...
        try:
            if not check_steam_available(
                ...
                self._steam_pool_error = "Steam is not available"
                return
            init_lock = Lock()
            self._steam_pool = Pool(
                processes=num_processes,
                initializer=_pool_init_worker,
                initargs=(project_root, libs_path, init_lock),
            )
            self._emit_message(f"\nSteamworks Pool ready ({num_processes} workers)")
        except Exception as e:
            self._steam_pool_error = str(e)
            logger.exception("Failed to set up Steamworks pool")
        finally:
            self._steam_pool_ready.set()
    ...
    def create_steam_db(
        ...
        Parallelism: Steamworks Pool setup (check_steam_available + worker init)
        runs in a background thread during the Web API rounds so it's ready
        to accept work as soon as pfids are known.
        ...
        # Start Steamworks Pool setup in background while Web API rounds run
        ...
        if self.get_appid_deps:
            ...
            self._emit_message(
                f"\nAppID dependency retrieval enabled. Setting up Steamworks Pool "
                f"({num_processes} workers) in background during Web API"
            )
            Thread(
                target=self._setup_steamworks_pool,
                args=(num_processes, libs_path, project_root),
                daemon=True,
            ).start()
        ...
        if self.get_appid_deps:
            ...
            if self._steam_pool_error or self._steam_pool is None:
                self._emit_message(
                    "\nAppID dependency retrieval skipped due to Steamworks error"
                )
            else:
                pool = self._steam_pool
                self._emit_message(
                    f"\nQuerying AppID dependencies via Steamworks for {len(publishedfileids)} mods"
                )
                steamworks_all_results.update(
                    self._run_steamworks_batch(publishedfileids, query, pool)
                )
                if missing_children:
                    remaining = [
                    ...
                    if remaining:
                        steamworks_all_results.update(
                            self._run_steamworks_batch(remaining, query, pool)
                        )

                self._merge_app_deps(query, steamworks_all_results)
                pool.close()
                pool.join()

                api_calls = sum(len(v) for v in steamworks_all_results.values())
                self._emit_message(
                    f"\nAppID dependencies resolved: {api_calls} DLC entries"
                )
    ...
    def IPublishedFileService_GetDetails(
        ...
        self._emit_message(
            f"IPublishedFileService/GetDetails chunk [0/{total}]{self._steamworks_status()}"
        )
        if not self.api:
            return None
        all_details: list[dict[str, Any]] = []
        chunk_list = list(chunks(publishedfileids, limit=200))
        # Chunk limit appears to be 213 PublishedFileIds at a time
        _api = self.api  # local ref for closure type safety
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            future_to_chunk = {
                executor.submit(_fetch, chunk): chunk for chunk in chunk_list
            }
            for future in as_completed(future_to_chunk):
                ...
                try:
                    ...
                    if (
                        ...
                        stacktrace = stacktrace[
                            ...
                            - (
                                len(stacktrace)
                                - (stacktrace.find(pattern) + len(pattern))
                            )
                        ]
                    logger.error(
                ...
                self._emit_message(
                    f"IPublishedFileService/GetDetails chunk [{items_processed}/{total}]{self._steamworks_status()}"
                )
    ...
    def IPublishedFileService_QueryFiles(self, cursor: str) -> str:
        ...
        for item in response_data.get("publishedfiledetails", []):
            ...
            if pfid:
                self.publishedfileids.append(pfid)
                ids_from_page.append(pfid)
        self.pagenum += 1
    ...
    def _run_steamworks_batch(
        ...
        the pre-created Pool. Filters out pfids that already have DLC deps.

        :param publishedfileids: pfids to query (will be filtered)
        :param query: database dict used to check existing deps
        :param pool: pre-created multiprocessing.Pool
        :return: dict mapping pfid_int -> list of app_id ints
        """
        pfids_needing_deps = [
            pfid
            ...
            if any(
                str(a)
                not in query.get("database", {}).get(pfid, {}).get("dependencies", {})
                for a in RIMWORLD_DLC_METADATA
            )
        ]
        ...
        queries = [
            ...
            for chunk in chunks(pfids_needing_deps, limit=CHUNK_SIZE)
        ]

        total_chunks = len(queries)
        COOLDOWN_INTERVAL = 6000
        pfids_processed = 0
        start_time = time()
        pfids_appid_deps: dict[int, list[int]] = {}

        self._emit_message(
            f"IPublishedFileService/GetDetails chunk [0/{total_chunks}] | Steamworks"
        )
        for i, result in enumerate(
            ...
            est_remaining = int(elapsed / done * remaining) if done else 0
            if est_remaining < 60:
                est_str = f"{est_remaining}s"
            else:
                est_str = f"{est_remaining // 60}m {est_remaining % 60}s"
            self._emit_message(
                f"IPublishedFileService/GetDetails chunk [{done}/{total_chunks}] "
                f"| Steamworks | est: ~{est_str}"
            )
    ...
    def ISteamUGC_GetAppDependencies(
        ...
        Legacy entry point — delegates to _run_steamworks_batch.

        Note: Does NOT close/join the pool — the caller owns pool lifetime.
        """
        result = self._run_steamworks_batch(publishedfileids, query, pool)
        if result:
            self._merge_app_deps(query, result)
            if self.output_database_path:
                try:
                    atomic_json_dump(query, self.output_database_path, indent=4)
                except Exception as e:
...
def ISteamRemoteStorage_GetPublishedFileDetails(
    ...
    for chunk in list(chunks(_list=publishedfileids, limit=5000)):
        ...
        try:
            ...
            logger.error(
                f"GetPublishedFileDetails chunk [{items_processed}/{total}]: {error_desc}"
            )
        ...
        if request.status_code >= 400:
            error_desc = f"Steam API returned HTTP {request.status_code}"
            logger.error(
                f"GetPublishedFileDetails chunk [{items_processed}/{total}]: "
                f"{error_desc} (non-retryable or retries exhausted)"
            )
            failed_pfids.extend(chunk)
            errors.append(f"{error_desc} for {chunk_size} mods")
            continue
        ...
        try:
            ...
            error_desc = f"Invalid JSON response (HTTP {request.status_code})"
            logger.error(
                f"GetPublishedFileDetails chunk [{items_processed}/{total}]: "
                f"{error_desc}: {e}"
            )
