...
class DynamicQuery(QObject):
    ...
    def create_steam_database(
        ...
        while querying:  # Begin initial query
            ...
            if result is None:
                logger.warning(
                    f"Dynamic Query failed to initialize WebAPI query! Critical failure. Aborting steam_database creation. query: {query} publishedfileids: {publishedfileids}"
                )
                return
            ...
            if (
                ...
                if result is None:
                    logger.warning(
                        f"Dynamic Query failed to initialize WebAPI query! Critical failure. Aborting steam_database creation. Query: {query} Missing Children: {missing_children}"
                    )
                    return
