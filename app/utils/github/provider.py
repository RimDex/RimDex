...
class GitHubProvider:
    ...
    def _get_cached_releases(
        ...
        last_checked_raw = entry.last_checked
...
def create_provider(settings: object) -> GitHubProvider:
    """Create a GitHubProvider using settings and the global cache DB.

    Centralizes the repeated pattern of ``GitHubProvider(token, cache_session)``
    scattered across handlers and panels.
    """
    from app.core.app_info import AppInfo
    from app.utils.github.models import get_cache_session

    cache_session = get_cache_session(
        AppInfo().app_storage_folder / "github_release_cache.db"
    )
