...
class GitHubInstaller:
    ...
    def install_head(repo_url: str, target_dir: str) -> tuple[bool, str | None]:
        ...
        Uses the project's existing :mod:`app.git.git_utils` infrastructure.
    ...
    def backup_mod(mod_path: Path) -> Path:
        """Move a mod directory to a ``.rimdex_backup`` sibling.
        ...
        """
        backup_path = mod_path.parent / f"{mod_path.name}.rimdex_backup"
    ...
    def restore_backup(backup_path: Path, mod_path: Path) -> None:
        ...
        :param backup_path: Path to the ``.rimdex_backup`` directory.
