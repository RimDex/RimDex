...
``RESTRUCTURING.md`` (P1 — split ``git/git_utils.py`` (operations vs. UI/status
...
Anything that is purely a user-facing concern (notification handlers, the
configuration dataclass, the centralised error-to-dialog helper) lives in
:mod:`app.git.git_notifications` instead.  :mod:`app.git.git_utils` remains
as a thin re-export shim so that existing callers keep working.

The split keeps the operation layer free of Qt dialogs (``InformationBox``)
and notification logic, which makes the helpers easier to test and easier
to reuse from contexts that do not have a UI (e.g. background workers).
"""
...
from app.core.fs_utils import delete_files_with_condition
...
# app/utils/pygit2_loader.py and https://github.com/RimDex/RimDex/issues/2234.
from app.git.pygit2_loader import (
    CheckoutStrategy,
    Repository,
    ResetMode,
    SortMode,
...
# ---------------------------------------------------------------------------
# Low-level helpers (fetch / corruption / repair)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Repository context manager + discovery
# ---------------------------------------------------------------------------
...
def git_check_updates(
    ...
    try:
        ...
        _handle_git_error(
            ...
            repo_url=remote_url,
            repo=repo,
            repair_callback=_attempt_repository_repair,
        )
        return None
