...
class ModDeletionMenu(QMenu):
    ...
    def _sync_remove_from_uuids_with_selected_mods(
        self, selected_mods: list[ModMetadata] | None = None
    ) -> None:
    ...
    def _has_steam_mods(self, selected_mods: list[ModMetadata]) -> bool:
        """Check if any selected mods are Steam Workshop mods."""
        return any(mod.get("publishedfileid") for mod in selected_mods)

    def _is_steam_client_integration_enabled(self) -> bool:
        """Check if Steam client integration is enabled in settings."""
        return self.settings.instances[
            self.settings.current_instance
        ].steam_client_integration

    def _has_dds_files(self, selected_mods: list[ModMetadata]) -> bool:
        ...
        for mod in selected_mods:
            ...
            if mod_path:
                ...
                if textures_dir.is_dir():
                    try:
                        if any(textures_dir.rglob(f"*{self.DDS_EXTENSION}")):
                            return True
    ...
    def _refresh_actions(self) -> None:
        ...
        selected_mods = self.get_selected_mod_metadata()
        has_dds = self._has_dds_files(selected_mods)
        steam_enabled = self._is_steam_client_integration_enabled()
        has_steam = self._has_steam_mods(selected_mods)
        can_do_steam_action = steam_enabled and has_steam
        for q_action, fn in self.delete_actions:
            if fn == self.delete_dds_files_only:
                q_action.setEnabled(has_dds)
            elif fn in (
                self.delete_mod_and_unsubscribe,
                self.delete_mod_and_resubscribe,
            ):
                q_action.setEnabled(can_do_steam_action)
            else:
    ...
    def _perform_deletion_operation(
        self,
        selected_mods: list[ModMetadata],
        ...
        Args:
            selected_mods: List of selected mod metadata to operate on
        ...
        self._sync_remove_from_uuids_with_selected_mods(selected_mods)
    ...
    def delete_mod_completely(self) -> None:
        ...
        self._perform_deletion_operation(
            ...
            confirmation_text=self.tr(
                f"You have selected {len(selected_mods)} mod(s) for complete deletion."
            ),
    ...
    def delete_dds_files_only(self) -> None:
        ...
        self._perform_deletion_operation(
            ...
            confirmation_text=self.tr(
                f"You have selected {len(selected_mods)} mod(s) for DDS texture deletion."
            ),
            confirmation_info=self.tr(
    ...
    def delete_mod_keep_dds(self) -> None:
        ...
        self._perform_deletion_operation(
            ...
            confirmation_text=self.tr(
                f"You have selected {len(selected_mods)} mod(s) for selective deletion."
            ),
            confirmation_info=self.tr(
    ...
    def _handle_steam_action(
        ...
        # Translate action-specific strings
        if action == "unsubscribe":
            action_text = self.tr("unsubscribe")
            action_cap = self.tr("Unsubscribe")
        else:
            action_text = self.tr("resubscribe")
            action_cap = self.tr("Resubscribe")
        ...
        try:
            ...
            # Show success message
            show_information(
                title=self.tr("Steam {action}").format(action=action_cap),
                text=self.tr(
            ...
            show_warning(
                title=self.tr("{action} Error").format(action=action_cap),
                text=self.tr(
    ...
    def _delete_mods_and_manage_steam(self, action: str) -> None:
        ...
        if action == "unsubscribe":
            action_capitalized = self.tr("Unsubscribe")
            action_past = self.tr("unsubscribed")
        else:
            action_capitalized = self.tr("Resubscribe")
            action_past = self.tr("resubscribed")

        if self._confirm_deletion(
            self.tr("Confirm Deletion and {}").format(action_capitalized),
            self.tr(
                "You have selected {} mod(s) for deletion.<br>{} of these are Steam Workshop mods that will also be {}."
            ).format(selected_count, steam_count, action_past),
            self.tr(
                "<br>This operation will:<br>• Delete the selected mod directories from your filesystem<br>• {} Steam Workshop mods from your Steam account<br><br>Do you want to proceed?"
            ).format(action_capitalized),
        ):
            # Synchronize remove_from_uuids with current selected mods before deletion
            self._sync_remove_from_uuids_with_selected_mods(selected_mods)
