...
class MenuBar(QObject):
    ...
    def _create_file_menu(self) -> QMenu:
        ...
        self.upload_submenu = self._create_logfile_submenu(
            self.tr("Upload Log"), self.upload_log_actions
        )
        ...
        if self.settings.text_editor_location:
            ...
            self.default_open_logs_submenu = self._create_logfile_submenu(
                self.tr("Open Log in Default Editor"), self.default_open_log_actions
            )
    ...
    def _create_tools_menu(self) -> QMenu:
        tools_menu = self.menu_bar.addMenu(self.tr("Tools"))
        self.database_builder_action = self._add_action(
            tools_menu, self.tr("Database Builder…")
        )
        tools_menu.addSeparator()
        self.translation_manager_action = self._add_action(
            tools_menu, self.tr("Translation Manager…")
        )
        return tools_menu
