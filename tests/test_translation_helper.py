...
class TestAutoTranslateFile:
    ...
    def mock_xml_parsing(self, mocker: Any) -> Dict[str, Any]:
        ...
        # auto_translate_file saves via save_ts_file (real ET.tostring + temp file
        # replace); patch it so no real .ts file is touched and we can assert on it.
        mock_save = mocker.patch("translation_helper.save_ts_file")
        return {
            "parse": mock_parse,
            "tree": mock_tree,
            "root": mock_root,
    ...
    async def test_auto_translate_file_success(
        ...
        mock_xml_parsing["save"].assert_called_once_with(
            mock_xml_parsing["tree"], Path("locales/zh_CN.ts")
        )
    ...
    async def test_auto_translate_file_dry_run(
        ...
        mock_xml_parsing["save"].assert_not_called()
    ...
    async def test_auto_translate_file_failure_no_continue(
        ...
        mock_xml_parsing["save"].assert_not_called()  # File should not be saved
        ...
        #       we verify that save_ts_file is not called and the success is False.
    ...
    async def test_auto_translate_file_failure_with_continue(
        ...
        mock_xml_parsing["save"].assert_called_once()  # File should be saved
    ...
    async def test_auto_translate_file_unexpected_exception(
        ...
        mock_translation_service.translate.assert_called_once()
        # With continue_on_failure=True (default), save_ts_file IS called even when error occurs
        # because the error happens during translation and failed items are handled gracefully
        # File gets saved with partial results
        mock_xml_parsing["save"].assert_called_once()
