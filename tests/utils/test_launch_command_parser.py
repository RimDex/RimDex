...
from app.core.launch_command_parser import ParsedLaunchCommand, parse_launch_command
...
class TestParseWindowsBackslashPaths:
    ...
    @patch("app.core.launch_command_parser.sys")
    def test_backslash_path_preserved(self, mock_sys: MagicMock) -> None:
        """Backslash paths should not be mangled on Windows."""
        mock_sys.platform = "win32"
        result = parse_launch_command(
            r"-popupwindow -logfile C:\Users\lione\AppData\Local\RimDex"
        )
        assert result.game_args == [
            "-popupwindow",
            "-logfile",
            r"C:\Users\lione\AppData\Local\RimDex",
        ]

    @patch("app.core.launch_command_parser.sys")
    def test_savedatafolder_backslash_path(self, mock_sys: MagicMock) -> None:
        mock_sys.platform = "win32"
        result = parse_launch_command(
    ...
    @patch("app.core.launch_command_parser.sys")
    def test_quoted_path_with_spaces(self, mock_sys: MagicMock) -> None:
        mock_sys.platform = "win32"
    ...
    @patch("app.core.launch_command_parser.sys")
    def test_command_placeholder_with_backslash_args(self, mock_sys: MagicMock) -> None:
        mock_sys.platform = "win32"
        result = parse_launch_command(
    ...
    @patch("app.core.launch_command_parser.sys")
    def test_unc_path_preserved(self, mock_sys: MagicMock) -> None:
        mock_sys.platform = "win32"
