...
def dq() -> DynamicQuery:
    ...
    with patch.object(DynamicQuery, "__init__", lambda self: None):
        ...
        obj.total = 0
        obj.output_database_path = ""
        obj._steam_pool_ready = Event()
        obj._steam_pool_ready.set()
        obj._steam_pool_error = None
        obj._steam_pool = None
        return obj
...
class TestCreateSteamDbCaching:
    ...
    def test_appid_deps_receives_all_pfids(self, dq: DynamicQuery) -> None:
        """_run_steamworks_batch receives initial + missing pfids."""
        mods = {
            "A": _make_mod("A", "Mod A", children=["C"]),
            "C": _make_mod("C", "Mod C"),
        }
        ...
        dq._steam_pool = MagicMock()

        with (
            patch.object(dq, "_setup_steamworks_pool"),
            patch.object(dq, "_run_steamworks_batch", return_value={}) as mock_batch,
            patch.object(dq, "_merge_app_deps"),
        ):
            dq.create_steam_db(database=_make_database("A"), publishedfileids=["A"])
            assert mock_batch.call_count == 2
            all_pfids = set()
            for call in mock_batch.call_args_list:
                all_pfids.update(call[0][0])
            assert all_pfids == {"A", "C"}
