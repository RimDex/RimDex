...
class TestAppInfoAppImage:
    def test_is_appimage_when_env_set(
        ...
        fake_appimage = tmp_path / "RimDex.AppImage"
        fake_appimage.touch()
    ...
    def test_bak_cleanup_on_startup(
        ...
        fake = tmp_path / "RimDex.AppImage"
        fake.touch()
        bak = tmp_path / "RimDex.AppImage.bak"
...
SAMPLE_ASSETS: list[dict[str, Any]] = [
    _make_asset(
        "RimDex-1.2.3-Ubuntu-22.04_x86_64.zip",
        "https://example.com/ubuntu.zip",
    ),
    _make_asset(
        "RimDex-1.2.3-x86_64.AppImage",
        "https://example.com/rimdex.AppImage",
    ),
    _make_asset(
        "RimDex-1.2.3-Windows_x86_64.zip",
...
SAMPLE_ASSETS_WITH_TAR_GZ: list[dict[str, Any]] = [
    _make_asset(
        "RimDex-1.2.3-Ubuntu-22.04_x86_64.tar.gz",
    ...
    _make_asset(
        "RimDex-1.2.3-Darwin_arm.tar.gz",
...
class TestAssetSelection:
    ...
    def _linux_update_manager(self, monkeypatch: pytest.MonkeyPatch) -> UpdateManager:
        ...
        monkeypatch.setenv("APPIMAGE", "/home/user/RimDex.AppImage")
    ...
    def test_prefers_appimage_when_running_as_appimage(
        ...
        assert result["url"] == "https://example.com/rimdex.AppImage"
...
class TestPrepareAppImageUpdate:
    def test_writes_new_appimage_file(
        ...
        expected = tmp_path / "RimDex.AppImage.new"
    ...
    def test_raises_when_no_appimage_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from app.core.update_utils import UpdateExtractionError
    ...
    def test_raises_when_no_write_permission(
        ...
        fake = tmp_path / "readonly" / "RimDex.AppImage"
        fake.parent.mkdir()
        fake.touch()
        monkeypatch.setenv("APPIMAGE", str(fake))
...
def _run_tar_thread(tar_path: Path, extract_dir: Path) -> tuple[bool, str]:
    ...
    from app.core.update_utils import TarExtractThread
...
class TestTarExtractThread:
    ...
    def _create_tar_gz_file(self, tmp_path: Path, name: str = "RimDex") -> Path:
    ...
    def test_extracts_content(self, tmp_path: Path) -> None:
        ...
        assert (extract_dir / "RimDex").exists()
