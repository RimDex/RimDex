...
class TestMenuBarUpdateMenuCreation:
    ...
    def test_update_menu_shown_when_env_var_not_set(
        ...
        """Test that Update menu is created when RIMDEX_DISABLE_UPDATER is not set."""
        # Ensure environment variable is not set
        with patch.dict(os.environ, {}, clear=False):
            if "RIMDEX_DISABLE_UPDATER" in os.environ:
                del os.environ["RIMDEX_DISABLE_UPDATER"]
    ...
    def test_update_menu_hidden_when_env_var_set(
        ...
        """Test that Update menu is not created when RIMDEX_DISABLE_UPDATER is set."""
        # Set the environment variable
        with patch.dict(os.environ, {"RIMDEX_DISABLE_UPDATER": "1"}):
            qt_menu_bar = QMenuBar()
            menu_bar = MenuBar(qt_menu_bar, mock_settings_controller)
...
class TestDisableUpdaterFlag:
    ...
    def test_disable_updater_flag_sets_env_var(self) -> None:
        ...
        original_env = os.environ.get("RIMDEX_DISABLE_UPDATER")

        try:
            sys.argv = ["rimdex", "--disable-updater"]
            ...
            if original_env is None:
                ...
                os.environ["RIMDEX_DISABLE_UPDATER"] = original_env
    ...
    def test_multiple_disable_updater_flags(self) -> None:
        ...
        try:
            sys.argv = ["rimdex", "--disable-updater", "--disable-updater", "build-db"]
            ...
            assert "--disable-updater" not in sys.argv
            assert sys.argv == ["rimdex", "build-db"]
