...
RimDex 使用 [PySide6](https://pypi.org/project/PySide6/) 模块以及多个其他 Python 模块构建，部分模块需要特殊处理才能正确被编译。最终，使用 [Nuikta](https://nuitka.net/) 进行打包。

## 克隆仓库

- RimDex 使用了 Git 子模块，代码托管在其他仓库，需同步克隆：
- 带子模块克隆：`git clone --recurse-submodules -j8 https://github.com/RimDex/RimDex`
...
- RimDex 还依赖以下必需子模块（执行 `git submodule update --init --recursive` 来初始化/更新）：
  - [steamfiles](https://github.com/RimDex/steamfiles)：用于解析 Steam 客户端 acf/appinfo/manifest 文件
  - [SteamworksPy](https://github.com/philippj/SteamworksPy)：用于实现与本地 Steam 客户端的交互
    ...
    - 这使 RimDex 可以通过 Python 调用 Steamworks API，与本地 Steam 客户端进行交互（例如通过 RimDex 订阅/取消订阅 Steam Mod）

### 配置 Python 及依赖项

- RimDex 使用 Python 开发，需要多个 Python 模块支持。你可以通过 `requirements.txt` 文件安装/查看大部分依赖项。在项目根目录执行以下命令可一次性安装所有依赖：
...
- **steamfiles** 和 **SteamworksPy** 依赖因特殊原因无法通过 requirements.txt 直接安装
  ...
  - 参阅各自章节了解配置方法，或使用 `distribute.py` 自动安装。默认情况下该脚本会直接构建 RimDex，但可通过参数配置启用/禁用构建等步骤。使用 `python distribute.py --help` 查看详细说明
...
- 要让 RimDex 实际使用 SteamworksPy 模块，你需要准备对应平台的编译库文件，以及 steamworks SDK 的二进制文件放置在 RimDex 项目根目录下，配合位于 `SteamworksPy/steamworks` 的 Python 模块使用。
...
- RimDex 使用 [todds](https://github.com/joseasoler/todds) 作为纹理优化的依赖项。该工具已随 RimDex 发布版本打包。如果你从源码构建/运行 RimDex，需要将 todds 的二进制文件放置在以下路径：
