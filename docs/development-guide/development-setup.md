...
RimDex is built in Python using the [PySide6](https://pypi.org/project/PySide6/) module, as well as several others. Some modules require special care in order to be built. It is compiled and packaged using [Nuikta](https://nuitka.net/).

## Prerequisites
### OS
RimDex presently runs on Windows, MacOS, and Linux, though we presently only create builds for Ubuntu. It may work for other Linux distributions, but Ubuntu is our baseline.
...
RimDex uses submodules that are hosted in other repositories that need to be cloned. 
- [steamfiles](https://github.com/RimDex/steamfiles): used to parse Steam client acf/appinfo/manifest information
- [SteamworksPy](https://github.com/philippj/SteamworksPy): used for interactions with the local Steam client
  ...
  - This allows certain interactions with the local Steam client to be initiated through the Steamworks API via Python (such as subscribing/unsubscribing to/from Steam mods via RimDex)

To clone with submodules run:
```shell
git clone --recurse-submodules -j8 https://github.com/RimDex/RimDex
```
...
RimDex uses the Python package and project manager [uv](https://docs.astral.sh/uv/).
...
- For a (mostly automated) experience building RimDex, please execute the provided script:
  - Run `uv run python distribute.py`
    - This will build RimDex for your platform and output a build for your platform (Including all requirements and submodules)
...
### Setting up additional dependencies
 RimDex uses Python, and depends on several Python modules. You can install/view most of the above dependencies via `pyproject.tom`. These would have been set up in the prior environment setup step. However, the **SteamworksPy** dependency is a special case that has requires special handling. 

See their respective sections for information on how to set them up. Alternatively, use `distribute.py` to do so automatically. By default, the script will build RimDex, but it can be configured to enable or disable various steps including building. See `uv run python distribute.py --help` for more info.
...
- Mac users should also keep in mind that Apple has its own Runtime Protection called [Gatekeeper](https://support.apple.com/guide/security/gatekeeper-and-runtime-protection-sec5599b66df/web)
  - This can cause issues when trying to run RimDex (or execute dependent libs)!
...
For RimDex to actually USE the SteamworksPy module, you need the compiled library for your platform, as well as the binaries from the steamworks SDK in the RimDex project root - in conjunction the python module included at: `SteamworksPy/steamworks`.
...
{: .note}
> At the time of writing, the SteamworksPy module can only be built using Python 11, differing from RimDex itself. You may need to use a different Python environment from the one you use to handle RimDex.
...
- RimDex uses [todds](https://github.com/joseasoler/todds) as a dependency for texture optimization. It is shipped with RimDex, archived into the binary releases. If you are building/running from source, you will want to place a todds binary at `./todds/todds` (for Linux/Mac) OR `.\todds\todds.exe` (for Windows)

### Running RimDex from source
...
After following all the prior steps, from the RimDex project root directory, first add the `SteamworksPy` submodule to the Python path:
