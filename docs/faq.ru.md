...
## macOS Gatekeeper / Windows Defender считает RimDex опасным

RimDex не вредоносен. Сборки на Python/Nuitka часто дают ложные срабатывания AV. Можно добавить исключение или проверить файл на VirusTotal.
...
Убедитесь, что Steam запущен и интеграция включена. На macOS известная проблема — запускайте RimWorld через Steam; список модов из RimDex сохраняется в `ModsConfig.xml`.
