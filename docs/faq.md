...
## macOS gatekeeper/Windows Defender tells me that RimDex is damaged/unsafe to run/is malware

RimDex is not malware and is safe to use. You can safely override any AV (Anti-virus) detections.

Unfortunately, because RimDex is compiled Python, it has a tendency to trigger false detections, especially for new releases. There are ways for us to mitigate these detections, but it requires expensive code signing certificates. If you are still unsure, you can scan the executable/files using virus total. Typically, there may be a few false detections, but the vast majority of scans will return negative.
...
## How do I update RimDex manually if the built-in updater fails?

Close RimDex, then download the latest release for your current release channel, operating system, and CPU architecture from the [RimDex releases page](https://github.com/RimDex/RimDex/releases). Do not use `Code > Download ZIP`, because that archive contains the source code rather than a runnable build. Extract archive-based releases into a new empty folder instead of merging them with the old installation; on macOS, replace the existing `RimDex.app`, and on Linux, replace the AppImage and make the new file executable with `chmod +x RimDex-*.AppImage`. Launch the new version before removing the old application files. Your settings and other RimDex data are stored separately in your operating system's application data directory, so leave that directory in place. See [Downloading and Installing](https://rimdex.github.io/RimDex/user-guide/downloading-and-installing) for platform-specific instructions.
...
## Why do I get a `Could not initialize Steam API` error when starting RimWorld from RimDex?

{: .note}
> This is a known common issue on macOS. As a workaround, launch RimWorld directly via Steam.

First, make sure you have `Steam client integration` enabled in RimDex's settings. Additionally, ensure that Steam is running and authenticated with the user that owns RimWorld.

If the previous steps did not work, then try launching RimWorld from Steam instead of RimDex as a workaround. Unless you are using special run arguments, the mod list you created in RimDex should be the one used by RimWorld even if launched via Steam directly. If you use custom run arguments, you may need to pass them via Steam if launching from Steam.
