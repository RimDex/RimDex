...
RimDex использует внешние базы для сортировки и зависимостей. Они не в релизе — есть инструменты установки и обновления. Для базовой работы не обязательны, но улучшают UX.
...
Правила порядка загрузки от сообщества. Авторам лучше указывать правила в `about.xml`, но не всегда. CR DB — публичный репозиторий дополнительных правил с комментариями, включая RimDex-специфичные (например **Force load at bottom of list**).
...
{: .important}
> GitHub-учётные данные в Advanced нужны только для PR из RimDex. Для клонирования публичных репозиториев — не обязательны.
