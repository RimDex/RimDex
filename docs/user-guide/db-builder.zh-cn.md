...
- 你的 Steam 账户需要至少消费 5 美元才能获得 Steam WebAPI 的常规访问权限。这是 RimDex 构建完整 Mod 依赖元数据的重要途径。
...
4. 在 RimDex 设置面板的 `数据库构建器（DS Builder）` 页，将密钥填入 `Steam API Key`。

数据库构建器有两种「包含」模式，可用于 RimDex 创建、管理、维护和更新 Steam 数据库（与 RimPy 的 db.json 格式兼容）。
...
1. 打开 RimDex 设置面板中的数据库构建器页，位于菜单 `文件 > 设置 > 数据库构建器（File > Settings > DB Builder）`。

2. 确保已完成上述 Steam WebAPI 密钥配置步骤。

3. （可选）配置数据库过期时间（单位：秒）。 该过期时间将用于生成你数据库的 version 字段，计算方式为数据库创建时的时间戳 + 设定时长。这会影响 RimDex 提示数据库过期的时机，默认值为 1 周。注意：此设置位于 `数据库（Databases）` 页。
