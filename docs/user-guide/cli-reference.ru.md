...
CLI RimDex для серверов и контейнеров:
...
Сборка Steam Workshop DB через WebAPI: имена, URL, зависимости, опционально DLC. JSON совместим с RimDex и RimPy.
...
|--------|------|---------|-------------|
| `--api-key TEXT` | String | (см. ниже) | WebAPI ключ или `RIMDEX_STEAM_API_KEY` |
...
2. `RIMDEX_STEAM_API_KEY` — рекомендуется
...
Укажите ключ: `--api-key`, `RIMDEX_STEAM_API_KEY` или в GUI.
