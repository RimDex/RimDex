...
if [ -n "${APPIMAGE:-}" ]; then
	...
	if [ ! -w "$APPIMAGE_DIR" ]; then
		...
		log_error "Move RimDex to a user-writable location or update manually."
		exit 1
	fi
...
# On macOS, adjust source if it contains a RimDex.app bundle so rsync copies the
# bundle contents rather than nesting RimDex.app inside the target .app directory.
if [ "$OS" = "Darwin" ] && [ -d "$UPDATE_SOURCE_FOLDER/RimDex.app" ]; then
	UPDATE_SOURCE_FOLDER="$UPDATE_SOURCE_FOLDER/RimDex.app"
