...
> Last updated: 2026-09-13 (re-verified against the full source tree on this
> date; substantially updated from the 2026-07-16/17 snapshot). File counts/LOC
...
> File count sanity: `just stats` (2026-09-13) reports `app/` = **222** `.py`
> files / 66,639 raw LOC / 54,483 curated LOC and `tests/` = **133** `.py` files
> / 22,329 raw LOC / 17,376 curated LOC. Curated LOC excludes blank/comment
> lines. ⚠️ The `just stats` recipe was **broken** — it referenced
> `scripts/stats.py`, which did not exist (the whole `scripts/` directory was
> missing from the tracked tree, contradicting §8.5's "Done"). The recipe was
> fixed on 2026-09-13 by restoring `scripts/stats.py`; `just stats` now works.
> Re-run `just stats` to re-sync the file/LOC numbers here.

## 0. Current Snapshot

Total `app/`: **54,483 curated LOC** (66,639 raw) across **222** `.py` files.
Layering is enforced by CI guards (`check_deferred_imports.py`,
`check_layer_violations.py`, `check_i18n_extraction.py`) plus
mypy/pyright/jscpd. `models/` → `controllers`/`views` is forbidden and
currently clean; `utils/globals.py` mutable globals were removed (replaced by
`core/app_context.py` DI).

Largest areas (curated LOC / raw LOC / file count — measured 2026-09-13):
...
`net/` 505 / 631 / 5 (incl. `__main__.py` 194 / 308 / 1).

Biggest single files (raw / curated LOC, all raw > 1k):
`core/update_apply.py` (2127/1677 — the self-update apply layer), `views/main_content_panel.py` (1968/1643 — already reduced from the original 2701 via handler extraction), `git/git_operations.py` (1767/1376), `views/mods_panel.py` (1703/1379), `views/settings_dialog.py` (1689/1345), `views/mixins/context_menu_mixin.py` (1648/1417), `views/player_log_panel.py` (1591/1368), `windows/rule_editor_panel.py` (1449/1241), `views/mod_info_panel.py` (1160/1001), `controllers/file_search_controller.py` (1147/848).
`controllers/main_content_controller.py` is now **165 curated LOC / 211 raw**
(down from 1870 after the handler extraction in §2 P1).

Tests: **1492** collected across `tests/` (17,376 curated LOC, 133 `.py` files).
Last full run (2026-09-13): **1478 passed, 14 skipped, 0 failed** — see §1 and
the gate table in §7.
...
- Split `views/main_content_panel.py` by remaining sub-feature (mod lists, settings, GitHub updates). Still the largest view (1968 raw / 1643 curated LOC).
- Split `views/mixins/context_menu_mixin.py` (1648 raw / 1417 curated) and
  `views/mods_panel.py` (1703 raw / 1379 curated) per §8.1.
- ~~Add a coverage floor (`just cov-gate`)~~ **DONE** — recipe present (see §2 P3).
- ~~Document the `EventBus` signal catalogue in `docs/architecture.md`~~ **DONE**
  — section exists at `docs/architecture.md:70`.
- Finish remaining leaf-layer tests (`ui/widgets/`, `cli/`, `core/*`,
  `services/*`, `git/` repo ops, `mods/db_builder*`).
- **`# type: ignore` count regressed (17 in `app/`, up from 12):** three
  un-audited suppressions have appeared since the 2026-07-17 inventory —
  `app/net/privatebin.py:44,48` (`[arg-type]`, `_hmac_sha256` args),
  `app/git/git_operations.py:398` (`[attr-defined]`,
  `repo._has_hanging_threads`), `app/io/xml.py:9` (`[attr-defined]`,
  `bs4.builder.LXMLTreeBuilderForXML`). Re-audit per §8.7/§8.9.
- `tests/` `# type: ignore` grew to 39 (from 26) as new window/view tests
  landed (notably `tests/windows/test_github_mods_panel.py` ×11) — see §8.7.1.

### Test results (last full run — 2026-09-13)

`pytest --doctest-modules --no-qt-log`: **1492 collected → 1478 passed, 14 skipped,
0 failed** (~39s). This supersedes every earlier stale count in this doc
(1231/1335/1284/1267/1140/1213/1230 — the latter entries were all
pre-2026-09-13). The documented history below (2026-07-12) is kept for context:
the 4 earlier failures in
...
Files > 1.4k LOC concentrate logic that should be decomposed (raw / curated):

- `core/update_apply.py` (2127/1677 — the self-update apply layer; was `update_utils.py` at 2285, now a 38-curated-LOC shim), `views/main_content_panel.py` (1968/1643 — down from 2701)
...
- `controllers/main_content_controller.py` was reduced to **165 curated LOC** (211 raw, from 1870)
...
- **434** `Any` usages across `app/` annotations (re-measured 2026-09-13 via AST;
  was 418 @ 2026-07-16, 522 @ mid-July). The vast majority are legitimate
  `dict[str, Any]` for heterogeneous JSON/ACF data and `list[Any]` instruction
  tuples from the Steamworks bridge; a smaller set are intentional mixin stubs in
  `windows/mixins/_shared.py` / `views/mixins/_shared.py`.
- **17** `# type: ignore` comments in `app/` (re-measured 2026-09-13; was 12 @
  2026-07-17, 59 before the mid-July audit). **The count regressed** —
  see §8.7/§8.9: 7 are the necessary `TrMixin` `[misc]` suppressions
  (`windows/mixins/_shared.py`), and 3 are **new, un-audited** suppressions that
  appeared after the 2026-07-17 inventory (`app/net/privatebin.py:44,48`,
  `app/git/git_operations.py:398`, `app/io/xml.py:9`). The rest are the
  documented Qt `eventFilter`/`QKeyEvent`/`resizeEvent` MRO/cast cases plus the
  missing-stub (`vdf`, `networkx`) and gitpython gaps.

### Tech-debt markers

- **17** real `TODO`/`FIXME`/`HACK` code markers (re-counted 2026-09-13;
  the 3 `AGENTS.md` doc-comment hits are excluded). Full file:line catalogue
  with intent is in **§8.6**. **Composition changed since 2026-07-16:** the
  `main_content_panel.py:1267` "restoring mod lists emits 0-count noise" marker
  is **gone** (the TODO was removed; only a plain comment remains at
  `main_content_panel.py:1408`), and a **new** marker appeared at
  `app/utils/log_setup.py:35` (`TODO(debt)`: make `session_history`
  configurable via settings). The 4× duplicated "let user configure window
  launch state and size" note is still one tracked issue (#3).
...
event_bus, schema, update_check, win_find_steam, app_info), `io/` (json_utils, files,
acf_utils), `mods/` (ignore_extensions, file_search, aux_db_utils, db_builder),
`services/` (mod_path_service, path_autodetect_service, window_manager,
instance_service, import_export_service), `git/` (git_worker). **1492 tests
collected → 1478 passed, 14 skipped, 0 failed** (2026-09-13 run — supersedes all
earlier figures: 1335/1318/1284/1267/1140/1213/1230). **Still no dedicated tests**
for: `ui/widgets/`, and remaining `core/*`, `git/` (repo ops beyond worker), plus
the CLI (`cli/` — `cli/translate` is covered by `tests/cli/test_translate.py`,
the rest of the subcommand group is not) and the fixers/IO builders. These
are the most pure/testable logic and the highest-ROI coverage targets
(on 2026-07-16/17 `core/update_check`, `core/win_find_steam`, `core/app_info`,
`services/window_manager`, `services/instance_service`,
`services/import_export_service`, and `mods/db_builder` gained dedicated tests —
see the leaf-layer test note in §2 P3).
...
- [ ] **Split `views/main_content_panel.py` (originally 2701) + `controllers/main_content_controller.py` (originally 1870)**
  - Status: **controller + view handler decomposition complete**.
    - Controller: decomposed into 4 handler modules under `app/controllers/handlers/`:
      ...
      (`main_content_controller.py`) was reduced to **146 LOC** (from 1870; now
      165 curated / 211 raw, re-measured 2026-09-13), keeping
    ...
    - View (`MainContent`): import/export, Steam/SteamCMD/Steamworks, and ZIP-mod
      ...
      shrank to **1645 LOC** (from 2701; now 1968 raw / 1643 curated,
      re-measured 2026-09-13 — the sub-feature split below is still open).
      Repeated SteamCMD-runner-active check and
...
- [x] **Resolve / track the `TODO`/`FIXME`/`HACK` markers** (re-counted 2026-07-16:
       **17** real code markers (the 3 `AGENTS.md` doc-comment hits excluded) —
...
- [x] **Add unit tests for untested leaf layers** (in progress — first batch done):
  ...
  ~90 new tests added. Current suite: **1492 collected → 1478 passed, 14 skipped,
  0 failed** (2026-09-13 run — supersedes the earlier stale 1284/1267/1140/1213/1335 counts).
  - Remaining leaf modules to cover: rest of `core/*`, `services/`, `cli/`,
    `git/git_worker` (repo ops), `mods/db_builder*`, `ui/widgets/*`. The heavier
    ones (Qt UI, real git repos, DB builders, CLI, Windows registry) need fixtures
    and are higher-risk.
...
4. ~~P2: typing + `generic.py` split + debt markers~~ **DONE**.
   5. **Remaining (2026-09-13):** `main_content_panel.py` sub-feature split
      (still 1968 raw LOC), `context_menu_mixin.py` + `mods_panel.py` splits
      (§8.1), remaining leaf-layer tests (`core/constants`, `git/git_worker`
      repo ops, `ui/widgets/*`, rest of `cli/`), re-audit the 3 new
      `# type: ignore` suppressions (§8.7), and re-verify `scripts/stats.py`
      exists (restored 2026-09-13 — it was missing and `just stats` was broken).
      `cli/translate` is covered by `tests/cli/test_translate.py`;
      `mods/db_builder` (orchestration) by `tests/mods/test_db_builder.py`;
      `services/instance_service`, `services/import_export_service`, and
      `services/window_manager` by dedicated tests. The `update_utils.py`
      split is **DONE** (see §2 P1 / §8.1). The coverage floor
      (`just cov-gate`) and EventBus signal catalogue are **DONE** (see §2 P3).
...
- `just stats` — re-measures `app/`/`tests/` file + LOC + `locales` counts so
  the numbers in §0/§1/§2 stay honest. **Restored 2026-09-13:** the recipe was
  broken (its `scripts/stats.py` was missing from the tree — the whole
  `scripts/` directory was absent); the script has been re-created.
...
2. **Avoid `# type: ignore` unless extremely necessary.** A `# type: ignore`
   ...
   - Re-run `grep "# type: ignore" app/` after any type-related change so the
     count in §8.7 stays honest. The intended trend is fewer suppressions over
     time (59 → 12 by 2026-07-17); note the count is currently **17** (three
     un-audited additions — see §8.7 G / §8.9). **Re-verify new suppressions
...
- `app/utils/` contains substantive modules (steam/*, github/*, rentry/*, workshop_utils,
  db_builder_thread) at 5.4k curated LOC across 34 files.
...
| `scripts/`                  | included                    | tracked source — `stats.py` restored 2026-09-13 (the dir was **missing** from the tree, breaking `just stats`); also hosts `check.py` + `mapping.py` |
| `submodules/`               | included                    | tracked source (git submodules) |
| `themes/`                   | included                    | tracked source |
| `.github/` `.githooks/`     | included                    | tracked config |
| `.continue/`                | **excluded**                | ignored via root `.gitignore` IDE section (added 2026-07-16, §6.3) |
| `.vscode/` `.ropeproject/`  | **excluded**                | `.vscode/*` + `.ropeproject/` ignored; `.vscode/launch.json` + `settings.json` stay tracked |
| `__pycache__/`              | **excluded**                | `*.py[cod]` bytecode cache |
| `.pytest_cache/`            | **excluded**                | pytest cache |
| `.mypy_cache/`              | **excluded**                | mypy cache |
| `.ruff_cache/`              | **excluded** (root rule)    | promoted to root `.gitignore` 2026-07-16 (§6.3) |
| `.venv/`                    | **excluded**                | virtualenv |
| `.tools/`                   | **excluded**                | dev tooling — **not present** in tree right now |
| `.cocoindex_code/`          | **excluded**                | CocoIndex (`/.cocoindex_code/`) — **not present** right now |
| `build/`                    | **excluded**                | Nuitka build output — present |
| `todds/`                    | **excluded**                | `/todds` — present |
| `MagicMock/`                | **excluded**                | test artifact leak — present |
| `.translation_cache.json`   | **excluded**                | translation cache (root) — none present right now |
| `steamworks_sdk_164.zip`    | **excluded**                | `steamworks_sdk_*.zip` — none present right now |
...
| ``.coverage`` ``.coverage.*`` | coverage data files | `.coverage` present (post-`test-coverage`) |
...
| `/todds`                     | `todds/` | yes |
| `steamworks_sdk_*.zip`       | `steamworks_sdk_*.zip` | (none present right now) |
| `version.xml`                | generated by GH Actions | (none yet) |
| `ltex.dictionary*`           | LanguageTool dict | (none) |
| `.translation_cache.json`    | `.translation_cache.json` + `locales/.translation_cache.json` | (none present right now) |
| `MagicMock/`                 | `MagicMock/` | yes (test artifact leak — `mock.aux_db_path/*`) |
| `.tools/`                    | `.tools/` | (none present right now) |
| `/.cocoindex_code/`          | `.cocoindex_code/` | (none present right now) |
...
- [x] ~~`.vscode/`, `.ropeproject/`, `.continue/`~~ **Done (2026-07-16):** added
  ...
  the two tracked files remain versioned. Re-verified 2026-09-13: all three markers
  in this section are resolved and the `.gitignore` rules they produced are live.
...
| Type check (mypy)       | `just typecheck`                        | PASS — no issues in 365 source files |
| Type check (pyright)    | `just pyright`                          | PASS — 0 errors, 0 warnings, 0 informations |
| Copy-paste (jscpd)      | `just jscpd`                            | PASS — 0 clones (337 files, 94,994 lines) |
| Lint (ruff check)       | `uv run ruff check .`                   | PASS — clean |
| Format (ruff format)    | `uv run ruff format --check .`          | PASS — clean (416 files) |
| Test suite (pytest)     | `just test`                             | PASS — **1492 collected → 1478 passed, 14 skipped, 0 failed** |

**Summary:** the Windows `just check` gate (typecheck + pyright + jscpd +
deferred-imports + layer-check + i18n-check) is **fully green**, ruff
check/format are clean, and the pytest suite passes. The 2026-09-13 run
instantiated a fresh snapshot of §0 with `scripts/stats.py` restored (the
previous `just stats` was broken — the referenced script did not exist).
...
- [ ] **`views/main_content_panel.py` (1968 raw / 1643 curated LOC)** — still the largest view. Finish the
...
- [ ] **`views/mixins/context_menu_mixin.py` (1648 raw / 1417 curated LOC)** and **`views/mods_panel.py`
  (1703 raw / 1379 curated LOC)** — natural next targets after `main_content_panel.py`. The former is a
...
- `Any` is now **434** (re-measured 2026-09-13 via AST over `app/` annotations;
  was 418 @ 2026-07-16, 522 before the mid-July audit). The remaining bulk is
  `dict[str, Any]` /
...
- The `# type: ignore` reduction (59 → 11 via `TrMixin`, `HandlerViewProtocol`,
  `assign_event_handler`, `GitHubReleaseCache` `Mapped`, `BaseModsPanelSurface`)
  **held 12 @ 2026-07-17 but has since regressed to 17** (re-measured
  2026-09-13). Three un-audited suppressions appeared post-inventory:
  `app/net/privatebin.py:44,48` (`[arg-type]` on `paste_key` / `prf=` passed to
  the `PyNaCl`-style secretbox builder), `app/git/git_operations.py:398`
  (`[attr-defined]`, `repo._has_hanging_threads = True`), and
  `app/io/xml.py:9` (`[attr-defined]`, `bs4.builder.LXMLTreeBuilderForXML`).
  The remaining 14 are the previously-documented genuine gaps (see §8.7 and
  §8.9 for the current inventory).
...
- Remaining untested leaf modules (high ROI, mostly pure): `core/*` leftovers,
...
- ~~Keep §0/§1/§2 counts in sync with `git ls-files`~~ **Done (2026-07-16),
  BUT the script went missing — **restored 2026-09-13**. `just stats` calls
  `scripts/stats.py`; on 2026-09-13 the whole `scripts/` directory was absent
  from the tree, so `just stats` errored with "No such file or directory".
  `scripts/stats.py` was re-created (file/LOC + `locales/*.ts` + largest-file
  output) and `just stats` verified working — it now prints `app/` = 222 `.py`
  files / 66,639 raw LOC / 54,483 curated and `tests/` = 133 / 22,329 / 17,376.
  Re-created the script on this date to keep the documented workflow real;
  this doc's §0 figures were re-synced from it.
...
in `app/` (re-measured 2026-09-13 via `grep "#\s*(TODO|FIXME|HACK)"`; count is
**17**, unchanged from 2026-07-16 but the composition shifted — see the
removed/new entries below).
Excludes the 3 doc-comment references to `AGENTS.md` (in
...
- **Views**
  ...
  - _Removed since 2026-07-16:_ the `main_content_panel.py:1267` "restoring
    mod lists emits `Inactive mod count changed to: 0` noise" `TODO` is gone;
    only a plain comment survives at `main_content_panel.py:1408`.
...
- **Controllers**
  ...
  - `app/controllers/main_window_controller.py:168` — `TODO`: fix `@Slot()`-
    ...
    (https://bugreports.qt.io/browse/PYSIDE-2942). Note: the `@Slot()`
    decorator is currently commented out at this line.

- **Models**
  - `app/models/metadata/metadata_structure.py:712` — `TODO`: type out the
    `ModMetadata` keys with a `TypedDict` someday.

- **Utils / Steam + infra**
  - `app/utils/log_setup.py:35` — `TODO(debt)`: make `session_history`
    configurable via settings. **NEW since 2026-07-16.**
  ...
  - `app/utils/steam/db_builder_thread.py:112` — `TODO`: make this
    warning visible to the user.
  - `app/utils/steam/steamworks/wrapper.py:206` — `TODO`: rework for
    proper static type checking (Steamworks bridge).

- **Rule editor**
  - `app/windows/rule_editor_panel.py:953` — `TODO`: leaving the
...
**5** view markers (`main_content_panel.py` ×2 key-repeat,
...
**4** util/infra markers (`workshop_utils`, `db_builder_thread`,
`steamworks/wrapper`, `log_setup`), **1** rule-editor case-sensitivity.
Total **17** real markers (the 3 `AGENTS.md` doc-references are excluded);
line numbers were re-verified on 2026-09-13.

### 8.7 `# type: ignore` audit (2026-07-16; re-scanned 2026-09-13)
...
and `BaseModsPanelSurface` wins (count dropped **59 → 11/12**, then grew to
**17** with three post-inventory additions catalogued in the summary below). Every remaining
...
- `app/core/fs_utils.py:18` — `import vdf` (no type stubs for `vdf`).
...
- `app/windows/mixins/ui_mixin.py:175` — `key_event = QKeyEvent(event)`
  (`misc`: `QKeyEvent` ctor expects a `QEvent` but resolves loosely).
- `app/windows/mixins/ui_mixin.py:182` — `return super().eventFilter(...)`
  (`misc`: `TrMixin` base has no `eventFilter`, so `super()` resolves oddly;
  the real `QWidget.eventFilter` is reached at runtime via MRO).

**G. Post-2026-07-17 additions — 4 un-audited (actionable)**
- `app/net/privatebin.py:44,48` — `paste_key` / `prf=_hmac_sha256` passed to
  the secretbox builder (`[arg-type]`). **Task:** wrap in a typed
  `build_secretbox(...)` helper (or annotate the `prf` param/Callable) so the
  suppressions drop.
- `app/git/git_operations.py:398` — `repo._has_hanging_threads = True`
  (`[attr-defined]`). **Task:** type the pygit2 `Repository` surface via a
  Protocol/`# type: ...` on the attribute or set the flag through a small
  helper in `git_operations` itself.
- `app/io/xml.py:9` — `from bs4.builder import LXMLTreeBuilderForXML`
  (`[attr-defined]`). **Task:** re-check with `bs4` stubs; likely also
  resolvable with a `cast`/import-site annotation or a local `.pyi`.
...
runtime-correct). The count fell **59 → 12** by 2026-07-17, but is now **17**
(re-measured 2026-09-13): the 12 documented remnants plus **3 un-audited
suppressions** that landed after the 2026-07-17 inventory —
`G. app/net/privatebin.py:44,48` (`[arg-type]`, `paste_key` / `prf=` passed to
the secretbox builder), `app/git/git_operations.py:398` (`[attr-defined]`,
`repo._has_hanging_threads = True`), and `app/io/xml.py:9` (`[attr-defined]`,
`bs4.builder.LXMLTreeBuilderForXML`). The §8.9 backlog should treat these as
**actionable** first (wrap the PyNaCl call, type the `repo._hanging_threads`
flag, or add a local `.pyi` for `bs4.builder`). Current full `app/` inventory:
A×2 (missing stubs: `vdf`, `networkx`), C×1 (gitpython gap),
D residual ×1 (`resizeEvent` arg-type, kept), E×2 (Qt ctor/MRO casts),
G×4 (the three new sites above), plus the `TrMixin` ×7 `[misc]`.
B and F are DONE. Re-run `grep "# type: ignore" app/` after any change to keep
this count honest.
...
by pytest, not mypy). The 2026-07-16 scan measured **26** suppressions (down
from 31); re-scanning on 2026-09-13 finds **39**, because the test suite has
grown (1492 collected) — `tests/windows/test_github_mods_panel.py` ×11
(`panel._populate_from_mods`/`_get_selected_mod_data = MagicMock(...)`
`[method-assign]` and `editor_table_view.indexWidget` `[attr-defined]`),
`tests/views/test_main_content_divider_import.py` ×2 (`[assignment]`
SimpleNamespace), and new invalid-input paths are the additions. Buckets stay
legitimate and mostly **stay** — they exercise error/invalid-input paths,
MagicMock/stub dynamics, or module injection that the production types
...
- **Window/view mock & method-assign dynamics (keep)** —
  `tests/windows/test_github_mods_panel.py` ×11, `tests/views/test_main_window_close.py`
  ×2, `tests/views/test_main_content_divider_import.py` ×2, `io/test_dds_utility.py:25`
...
**Net:** `tests/` `# type: ignore` count is **39** (grown from 26 in step with
the +157 tests collected since 2026-07-17; all in the legitimate buckets above).
Not tracked in the `app/` gate; re-scan with `grep "# type: ignore" tests/` if
a future task widens the mypy scope to `tests/`.
...
**Current `app/` inventory (17, re-measured 2026-09-13) — keep vs actionable:**
...
- [ ] **`app/windows/mixins/ui_mixin.py:175` (bare) + `:182` `[misc]`.** Qt
...
- [ ] **`app/core/fs_utils.py:18` `import vdf` (bare).** No type stubs for
...
- [ ] **`app/net/privatebin.py:44,48` `[arg-type]` — NEW (post-2026-07-17).**
  `paste_key` / `prf=_hmac_sha256` into the secretbox builder. **Actionable:**
  wrap in a typed helper or annotate the `prf` Callable.
- [ ] **`app/git/git_operations.py:398` `[attr-defined]` — NEW.**
  `repo._has_hanging_threads = True`. **Actionable:** type the pygit2
  `Repository` surface (Protocol/helper).
- [ ] **`app/io/xml.py:9` `[attr-defined]` — NEW.**
  `from bs4.builder import LXMLTreeBuilderForXML`. **Actionable:** re-check
  with `bs4` stubs or a local `.pyi`.

**`tests/` inventory (39, re-measured 2026-09-13) — mostly keep, a few
actionable:**
- [ ] **Bare ignores in `tests/views/test_dialogue.py:70,191,198`** and
...
- [ ] **`tests/windows/test_github_mods_panel.py` ×11** — `MagicMock()`
  method-assign on panel members + `editor_table_view.indexWidget`
  `[attr-defined]`. **Actionable (optional):** a small typed panel-surface
  test-helper base could drop the `[attr-defined]` ones; the `[method-assign]`
  are the documented Qt-mock pattern and can stay.
...
**Priority:** the three new `app/` sites (§8.7 G) and the
`database_upload_handler` helper and the `tests/` bare-ignore → coded-ignore
pinning are the highest-ROI next steps; the `_shared.py`
